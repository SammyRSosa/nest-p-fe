  ...
  const assignWorker = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Error al asignar personal",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const removeWorker = async (workerId: string) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Error al remover personal",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const assignHead = async (headId: string) => {
    ...
    try {
      ...
      toast({
        title: "Jefe asignado",
        description:
          "El jefe del departamento ha sido actualizado exitosamente.",
      });
      onUpdate();
      onClose();
    } catch (error) {
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Error al asignar jefe",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="select-head">
                      Seleccionar trabajador como jefe
                    </SelectItem>{" "}
                    {[...department.workers, ...availableWorkers].map(
                      (worker) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          disabled={
                            loading || department.head?.id === worker.id
                          }
                        >
                          {loading ? (
                            <Loader2 className="h-4 w-4 animate-spin" />
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="select-worker">
                          Seleccionar trabajador para asignar
                        </SelectItem>{" "}
