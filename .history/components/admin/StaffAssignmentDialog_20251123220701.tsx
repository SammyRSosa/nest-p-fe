...
export function StaffAssignmentDialog({
  ...
  const fetchAvailableWorkers = async () => {
    try {
      ...
      // Filtrar trabajadores que no están asignados actualmente a este departamento
 ...
 const fetchAssignments = async () => {
  try {
    ...
    const departmentAssignments = await api.workerDepartments.getByDepartment(department.id);
  ...
  const assignWorker = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Error al asignar trabajador",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const removeWorker = async (assignmentId: string) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Error al remover trabajador",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
                  ...
                  {activeAssignments.map((assignment) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        {department.headOfDepartment?.worker.id === assignment.worker.id && (
                          <Badge variant="destructive">
                            <Crown className="h-3 w-3 mr-1" />
                            Jefe
                          </Badge>
                        ...
                        <Button
                          ...
                          disabled={loading || department.headOfDepartment?.worker.id === assignment.worker.id}
                        >
                          {loading ? (
