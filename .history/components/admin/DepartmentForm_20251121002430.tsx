...
import type { Department, DepartmentCreateRequest, DepartmentUpdateRequest, MedicationStockRequest, User } from "@/types"
...
export function DepartmentForm({ department, onSuccess, onCancel }: DepartmentFormProps) {
  ...
  const fetchWorkers = async () => {
    try {
      ...
      setWorkers(data.filter((worker: User) => 
        worker.role === 'head_of_department' || worker.role === 'doctor' || worker.role === 'nurse'
      ))
    } catch (error) {
  ...
  const updateMedication = (index: number, field: keyof MedicationStockRequest, value: any) => {
        ...
        <CardContent>
          {medications.length === 0 ? (
            <p className="text-muted-foreground text-center py-8">
              No hay medicamentos agregados. Los medicamentos se pueden gestionar después de crear el departamento.
            </p>
          ) : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateMedication(index, "currentQuantity", parseInt(e.target.value) || 0)}
                        min="0"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateMedication(index, "minQuantity", parseInt(e.target.value) || 0)}
                        min="0"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateMedication(index, "maxQuantity", parseInt(e.target.value) || 0)}
                        min="0"
                      />
                    </div>
